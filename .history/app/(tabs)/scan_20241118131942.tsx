...
const ScanScreen = () => {
  ...
  const loadModel = async () => {
    try {
      ...
      // Load the model and binary weights using Expo Asset
      const modelJson = require('../../assets/Model/model.json'); // Path to model.json
      const binFiles = [
      ...
      // Preload the assets (model and binary files)
      const assets = await Promise.all(binFiles.map(file => Asset.fromModule(file).downloadAsync()));

      // Load the model with the binary files
      const loadedModel = await tf.loadLayersModel(
        bundleResourceIO(modelJson, assets.map(asset => asset.localUri))
      );
  ...
  return (
    <View style={styles.container}>
      ...
      {prediction && !loading && <Text style={styles.prediction}>Prediction: {prediction}</Text>}
    </View>
  );
};
